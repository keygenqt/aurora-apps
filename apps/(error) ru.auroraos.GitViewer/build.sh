...
if [ -d "$PROJECT" ]; then
    ...
    git clone https://gitlab.com/omprussia/kmp/gitviewer.git
    cd $PROJECT
fi
...
# Build arm
$chroot mb2 --no-fix-version --target $target_arm build
mv ./RPMS/* $DIR/builds && git clean -fdx

# Build arm64
$chroot mb2 --no-fix-version --target $target_arm64 build
mv ./RPMS/* $DIR/builds && git clean -fdx

# Build x64
$chroot mb2 --no-fix-version --target $target_x64 build
mv ./RPMS/* $DIR/builds && git clean -fdx
